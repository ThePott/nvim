...
local function check_eof_scrolloff(ev)
	...
	local visual_distance_to_top = win_cur_line - 1
	...
	-- ADD THIS SECTION FOR TOP OF SCREEN:
	if visual_distance_to_top < scrolloff then
		local win_view = vim.fn.winsaveview()
		local new_topline = win_view.topline - (scrolloff - visual_distance_to_top)
		if new_topline < 1 then
			new_topline = 1 - (scrolloff - visual_distance_to_top)
		end
		vim.fn.winrestview({
			skipcol = 0,
			topline = new_topline,
